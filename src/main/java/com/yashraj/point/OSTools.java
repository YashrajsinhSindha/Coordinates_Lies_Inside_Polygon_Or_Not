...
public class OSTools
{
    ...
    **/
    ...
    /**
    *** Returns true if the OS is HP UX
    *** @return True if the OS is HP UX
    **/
    public static boolean isUnixHPUX()
    {
        return isOSType(OS_UNIX, OS_UNIX_HPUX);
    }
    ...
    **/
    public static OSTools.CpuUsage getCpuUsage(OSTools.CpuUsage cpu, boolean displayWarnings)
    {
        ...
        synchronized (OSTools.CpuLock) {
            ...
            java.lang.management.OperatingSystemMXBean opSysMXBean = null;
            Class<?> opSysMXBeanClass = null;
            try {
                opSysMXBean = java.lang.management.ManagementFactory.getOperatingSystemMXBean();
                opSysMXBeanClass = opSysMXBean.getClass();
            } catch (Throwable th) {
                if (displayWarnings) {
                    Print.logWarn("Unable to obtain cpu usage - " + th);
                }
                error = true;
            }
            ...
            try {
                ...
                //if (opSysMXBeanClass != null) {
                //    Method ap = opSysMXBeanClass.getDeclaredMethod("getAvailableProcessors");
                ...
                if (displayWarnings) {
                    Print.logWarn("Unable to obtain cpu available processors - " + th);
                }
                error = true;
            }
            ...
            if (opSysMXBeanClass != null) {
                try {
                    Method pcl = opSysMXBeanClass.getDeclaredMethod("getProcessCpuLoad");
                    ...
                    if (displayWarnings) {
                        Print.logWarn("Unable to obtain process cpu load - " + th);
                    }
                    error = true;
                }
            }
            ...
            if (opSysMXBeanClass != null) {
                try {
                    Method pct = opSysMXBeanClass.getDeclaredMethod("getProcessCpuTime");
                    ...
                    if (displayWarnings) {
                        Print.logWarn("Unable to obtain process cpu time - " + th);
                    }
                    error = true;
                }
            }
            ...
            if (opSysMXBeanClass != null) {
                try {
                    Method scl = opSysMXBeanClass.getDeclaredMethod("getSystemCpuLoad");
                    ...
                    if (displayWarnings) {
                        Print.logWarn("Unable to obtain system cpu load - " + th);
                    }
                    error = true;
                }
            }
            ...
            if (opSysMXBeanClass != null) {
                try {
                    Method scla = opSysMXBeanClass.getDeclaredMethod("getSystemLoadAverage");
                    ...
                    if (displayWarnings) {
                        Print.logWarn("Unable to obtain system cpu average load - " + th);
                    }
                    error = true;
                }
            }
            
            /* errors and displayWarnings? */
            if (error && displayWarnings && (opSysMXBeanClass != null)) {
                Print.logWarn("OperatingSystemMXBean declared methods:");
                Method M[] = opSysMXBeanClass.getDeclaredMethods();
                for (Method m : M) { 
                    Print.logWarn("   Method: " + m); 
                }
            }
            ...
            if (OSTools.isMacOSX()) {
                // -- attempt to read CPU information from command
                // -    /usr/sbin/sysctl -n machdep.cpu.brand_string
                // -    ==> "Intel(R) Core(TM) i7-4960HQ CPU @ 2.60GHz"
                StringBuffer outSB = new StringBuffer();
                StringBuffer errSB = new StringBuffer();
                String cmd[] = { "/usr/sbin/sysctl", "-n", "machdep.cpu.brand_string" };
                OSTools.exec(cmd, outSB, errSB);
                if (outSB.length() > 0) {
                    // -- look for "CPU @ 2.60GHz"
                    String s = outSB.toString();
                    int    p = s.indexOf("CPU");
                    if (p >= 0) { p = s.indexOf("@",p); }
                    if (p > 0) {
                        p++; // skip "@"
                        String cpuSp = s.substring(p).trim();
                        double speed = StringTools.parseDouble(cpuSp, 0.0);
                        double spMHz = cpuSp.endsWith("GHz")? (speed * 1000.0) : speed;
                        if (cpu == null) { cpu = new OSTools.CpuUsage(); }
                        cpu.setCpuSpeedMHz(spMHz);
                    }
                }
            } else
    ...
    **/
    ...
    private static Class<?> _getCallerClass(int frame)
        throws Throwable
    {
        return getCallerClass(frame + 1); // <== ignore any warnings
    }
    ...
    *** wrong parent PID on Linux. (thus this method is "private")
    ...
    *** Gets the Process-ID of this JVM invocation.<br>
    *** Attempts to obtain the PID of this process by executing a subprocess that requests
    *** that calls "ps h -p ppid $$".  This method does not work on Windows, and may not return the
    *** correct parent PID on Linux. (thus this method is "private")
    *** @return The PID of this process
    **/
    private static int _getProcessID2()
    {
        try {
            String cmd_ps = "ps"; 
            String cmd[] = new String[] { "bash", "-c", cmd_ps + " h -o ppid $$" }; // ps h -o
            Process ppidExec = Runtime.getRuntime().exec(cmd);
            BufferedReader ppidReader = new BufferedReader(new InputStreamReader(ppidExec.getInputStream()));
            StringBuffer sb = new StringBuffer();
            for (;;) {
    ...
    **/
    ...
    /**
    *** "renice" parent process
    **/
    /*
    private static reniceParent(String val)
    {
        String cmd_sh = "sh";
        String cmd_renice = "renice";
        String cmd_ps = "ps";
        String cmd = cmd_sh+" -c "+cmd_renice+" "+val+" `"+cmd_ps+" h -p ppid $$`";
    }
    */
        ...
        if (!ListTools.isEmpty(cmdArgs)) {
            ...
            try {
                ...
                if (cmdArgs.length > 1) {
                    ...
                    String cmd = StringTools.join(cmdArgs,' '); // cmdArg[0]
    ...
    **/
    ...
    /**
    *** Executes the specified command in a separate process
    *** @param envVars  The environment variables to add to the execution environment
    *** @param cmdArgs  The command and arguments to execute
    *** @param outSB    The StringBuffer into which the stdOut output is placed
    *** @param errSB    The StringBuffer into which the stdErr output is placed
    *** @return The process exit status code
    **/
    public static int exec(Map<String,String> envVars, String cmdArgs[], StringBuffer outSB, StringBuffer errSB)
    {
        int exitStat = -1;
        if (!ListTools.isEmpty(cmdArgs)) {
            Process process = null;
    ...
    **/
    ...
    {
        return OSTools.sleepSec((long)sec);
    }
    ...
    **/
    public static void main(String argv[])
    {
        ...
        Print.sysPrintln("PID #3      : " + _getProcessID2() + " (using subprocess 'ps h -o ppid $$')");
        // -- 
        Print.sysPrintln("");
