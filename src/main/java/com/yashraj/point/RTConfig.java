...
//     -Added Servlet "<Context>/webapp.conf" to "_getConfigURL(...)"
// ----------------------------------------------------------------------------
...
public class RTConfig
{
    ...
    **/
    public static PropertyGetter getPropertyGetter()
    {
        return new PropertyGetter() {
            public Object getProperty(Object key, Object dftValue) {
                if (key instanceof String) {
                    return RTConfig.getProperty((String)key, dftValue);
                } else {
                    return RTConfig.getProperty(StringTools.trim(key), dftValue);
                }
            }
        };
    }
    ...
    /**
    *** Gets an immutable RTProperties instance backed by the RTConfig properties.
    *** The exception "UnsupportedOperationException" will be thrown if any of the 
    *** following methods are sent to the backing map: 
    ***     containsValue, entrySet, values, clear, remove, put, putAll
    **/
    /* Not fully implemented
    public static RTProperties getRTProperties()
    {
        Map<Object,Object> backingMap = new Map<Object,Object>() {
            private boolean dftOK = true;
            private Set<Object> _keySet = null;
            public boolean containsKey(Object K) {
                return RTConfig.hasProperty(K, this.dftOK);
            }
            public Object get(Object K) {
                return RTConfig.getProperty(K,null);
            }
            public boolean isEmpty() {
                return (this.size() > 0)? false : true; // unlikely ever empty
            }
            public int size() {
            ...
            public Set<Object> keySet() {
                if (this._keySet == null) {
                    this._keySet = RTConfig.getPropertyKeys(null, this.dftOK);
                }
                return this._keySet;
            }
            public int hasCode() {
            ...
            public boolean containsValue(Object K) { throw new UnsupportedOperationException(); }
            public Set<Map.Entry<Object,Object>> entrySet() { throw new UnsupportedOperationException(); }
        ...
        return new RTProperties(backingMap);
    }
    */
    ...
    **/
    ...
    /**
    *** Returns the array of internal RTProperties
    **/
    public static RTProperties[] _getRTConfigProperties()
    {
        return CFG_PROPERTIES;
    }
    ...
    **/
    ...
    /**
    *** Gets the property value from the current Thread properties
    **/
    public static Object getThreadProperty(String key, Object dft)
    {
        RTProperties rtp = RTConfig.getThreadProperties(key);
        if (rtp != null) {
            return rtp.getProperty(key, dft);
        } else {
            return dft;
        }
    }
    ...
    **/
    public static Object getThreadProperty(String key[], Object dft)
    {
        if (key != null) {
            for (String k : key) {
                Object val = RTConfig.getThreadProperty(k, null);
                if (val != null) {
                    return val;
                }
            }
    ...
    /**
    *** Gets the String property value from the current Thread properties
    **/
    public static String getThreadString(String key, String dft)
    {
        Object val = RTConfig.getThreadProperty(key, null);
        if (val == null) {
            return dft;
        } else {
    ...
    **/
        ...
        throws MalformedURLException
    {
        ...
        //  - Servlet "<Context>/webapp.conf"                            (if servlet and specific context path)
        ...
        if (isTrueServlet) {
            ...
            if (!StringTools.isBlank(ctxPath)) {
                ...
                // -- Check for "<Context>/webapp.conf"
                File ctxFile = new File(ctxPath, cfgFileName);
                if (ctxFile.isFile()) {
                    //Print.logInfo("Servlet - Found at: " + ctxFile);
                    return FileTools.toURL(ctxFile);
                }
                // -- not found
    ...
    **/
    protected static synchronized void _startupInit(boolean allowChangeSystemProperties)
    {
        ...
        if (RTConfig.hasProperty(RTKey.DEBUG,false/*NoDefaults*/)) {
            ...
            RTProperties debugRTP = RTConfig.getPropertiesForKey(RTKey.DEBUG, false/*NoDefaults*/);
        ...
        if (merge) {
            ...
            if (CFG_THREAD_TEMPORARY != null) {
                ...
                if (rtpStack != null) {
                    ...
                    int level = 1;
                    for (int r = rtpStack.size() - 1; r >= 0; r--) {
                        Map<Object,Object> cp = rtpStack.get(r).getProperties();
                        String cpName = "ThreadTemp#"+(level++);
            ...
            /* get the current thread property stack */
            RTConfig.printThreadProperties("ThreadTemp");
    ...
    **/
    private static String _insertKeyValues(final Object mainKey, final String text, 
        ...
        final Map<String,String> customMap, final boolean rtOK)   // _insertKeyValues_main
    {
        //Print.logInfo("replacing vars: Text="+text+", MainKey="+mainKey+", Delim="+startDelim+"/"+endDelim+"/"+dftDelim);
        if (text != null) {
            ...
            StringTools.KeyValueMap rm = new StringTools.KeyValueMap() { // ReplacementMap
                ...
                public String getKeyValue(String k, String argNotUsed, String dft) {
                    // -- reset?
                    //Print.logInfo("Key="+k+", Arg="+argNotUsed+", Dft="+dft);
                    if (k == null) {
    ...
    *** the specified value
    *** @param startsWith  Search string
    *** @param inclDft     True to include default keys
    ...
    **/
    ...
    {
        return getDateTime(key, null, true);
    }

    // do not include this method, otherwise "getDateTime(key, null)" would be ambiguous
    //public DateTime getDateTime(String key, String dft)
    ...
    *** @return The <code>DateTime</code> value of the property
    **/
    public static DateTime getDateTime(String key, DateTime dft)
    {
        return getDateTime(key, dft, false);
    }

    /**
    *** Gets a <code>DateTime</code> property from a specified array of keys
    *** @param key  A property key.
    *** @param dft The default value to return if an entry was not found
    *** @param dftOK  True to check defaults, if not found elsewhere
    *** @return The <code>DateTime</code> value of the property
    **/
    public static DateTime getDateTime(String key, DateTime dft, boolean dftOK)
    {
        RTProperties rtp = RTConfig.getPropertiesForKey(key, dftOK);
        return (rtp != null)? rtp.getDateTime(key, dft) : dft;
    }
    ...
    **/
    public static DateTime getDateTime(String key[], DateTime dft, boolean dftOK)
    {
        if (key != null) {
            for (int i = 0; i < key.length; i++) {
                RTProperties rtp = RTConfig.getPropertiesForKey(key[i], dftOK);
                if (rtp != null) { return rtp.getDateTime(key[i], dft); }
            }
        }
        return dft;
    }
    ...
    /**
    *** Sets the <code>DateTime</class> property value for the specified key
    *** @param key  The property key
    *** @param value  The property value to set.
    **/
    ...
    {
        RTProperties cfgProps = getConfigFileProperties();
        cfgProps.setDateTime(key, value);
    }
    ...
    **/
    ...
    /**
    *** Gets the servlet class if this application is running as a servelet
    ...
    **/
    public static void main(String argv[])
    {
        ...
        if (RTConfig.hasProperty("test")) {
            Print.sysPrintln("");
            DateTime dt = RTConfig.getDateTime("dt",null);
            if (dt != null) {
                Print.sysPrintln("Date: " + dt + " ["+dt.getTimeSec()+"]");
            }
            // -- 
