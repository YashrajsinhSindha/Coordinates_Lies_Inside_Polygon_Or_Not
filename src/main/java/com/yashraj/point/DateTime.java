...
//     -Changed default day-start from 00:01:01 to 00:00:01 [2.6.7-B21b]
//     -Changed "toString()" date format (see TOSTRING_DATE_FORMAT) [2.6.7-B21b]
// ----------------------------------------------------------------------------
    ...
    implements Comparable<Object>, Cloneable
{
    ...
    public static final TimeZone MST                        = DateTime.getTimeZone("US/Mountain");
    public static final TimeZone CST                        = DateTime.getTimeZone("US/Central");
    public static final TimeZone EST                        = DateTime.getTimeZone("US/Eastern");
  ...
  //public static final String   TOSTRING_DATE_FORMAT       = "EEE MMM dd HH:mm:ss zzz yyyy"; // "Sun Mar 26 12:38:12 PST 2006" (not parsable by "parseArgumentDate")
    public static final String   TOSTRING_DATE_FORMAT       = "yyyy/MM/dd,HH:mm:ss,zzz";      // "2006/03/26,12:38:12,PST" [2.6.7-B21b]
    ...
    **/
    ...
    public static final long    SECONDS_PER_YEAR        = (long)((double)SECONDS_PER_DAY * 365.2425); // 31556952
    ...
    /** 
    *** Returns the (average) number of seconds in the specified number of years
    *** @param days  The number of years to convert to seconds
    *** @return The number of seconds
    **/
    public static long YearSeconds(int years)
    {
        return (long)years * SECONDS_PER_YEAR;
    }
    ...
    *** @return The number of seconds
    **/
    public static long YearSeconds(double years)
    {
        return Math.round(years * (double)SECONDS_PER_YEAR);
    }

    // --------------------------------

    /** 
    *** Returns the (average) number of seconds in the specified number of weeks
    *** @param days  The number of years to convert to seconds
    *** @return The number of seconds
    **/
    public static long WeekSeconds(int weeks)
    {
        return (long)weeks * SECONDS_PER_WEEK;
    }
    ...
    *** @return The number of seconds
    **/
    public static long WeekSeconds(long weeks)
    {
    ...
    *** @return The number of seconds
    **/
    public static long WeekSeconds(double weeks)
    {
        return Math.round(weeks * (double)SECONDS_PER_WEEK);
    }
  ...
  //public static final int  MAY                = MAY;
    ...
    **/
    ...
    /**
    *** Gets the Epoch start date/time (starting at 1 second)
    *** @return The Epoch start date/time
    **/
    public static DateTime getEpochDate()
    {
        int YYYY = EPOCH_YEAR;   // 1970
        int MM1  = EPOCH_MONTH1; // 1
        int DD   = EPOCH_DAY;    // 1
        return new DateTime(DateTime.GMT, YYYY, MM1, DD);
    }
    ...
    ***   YYYY-MM-DDThh:mm:ss[.ffff]Z        (ie. "2012-03-08T06:49:58.000Z" ISO-8601 - GPX date format)<br>
    ***   YYYY-MM-DDThh:mm:ss[.ffff]+XX:XX   (ie. "2012-03-08T06:49:58.000+00:00")<br>
        ...
        throws DateParseException
    {
        ...
        if (dateGrp.length == 1) {
            ...
            if ((dlen >= 19) && (dateStr.charAt(10) == 'T')) {
                ...
                //  YYYY-MM-DDThh:mm:ss[.ffff]+00:00
                String ds = dateStr.substring( 0,10); // YYYY-MM-DD
                String ts = dateStr.substring(11,19); // hh:mm:ss
                String tz = ""; // Z, +00:00
                if (dlen > 19) {
                    if (dateStr.charAt(19) == '.') {
                        // -- skip fractional seconds [TODO: should convert these to milliseconds?]
                        long millis = Math.round(StringTools.parseDouble(dateStr.substring(19),0.0) * 1000.0);
                        int p = 20;
                        while ((p < dlen) && Character.isDigit(dateStr.charAt(p))) { p++; }
                        tz = dateStr.substring(p);
                    } else {
                        // -- no fractional seconds
                        tz = dateStr.substring(19);    // Z, +00:00
                    }
                }
            ...
            if ((s == '+') || (s == '-')) {
                ...
                if (((DLen == 4) || (DLen == 3)) && !Character.isDigit(D.charAt(1))) {
                    ...
                    if (useCtxStart || useDayStart) {
                        hh =  0;
                        mm =  0; // day-start change minute 1==>0 [2.6.7-B21b]
                        ss =  1;
                    } else
                    ...
                    switch (lastCh) {
                        ...
                        case 'h': { // bours
                            ...
                            int DD   = dt.getDayOfMonth();
                            int hh   = dt.getHour24();
                            int mm   = (useDayStart||useCtxStart)? 0 : (useDayEnd||useCtxEnd)? 59 : dt.getMinute(); // day-start change minute 1==>0 [2.6.7-B21b]
                        ...
                        case 'm': { // months (ambiguous with 'minutes')
                            ...
                            int mm = (useDayStart||useCtxStart)? 0 : (useDayEnd||useCtxEnd)? 59 : now.getMinute(); // day-start change minute 1==>0 [2.6.7-B21b]
        ...
        if (grpLen == 2) {
            ...
            if ((s == '+') || (s == '-')) {
                ...
                if (((DLen == 4) || (DLen == 3)) && !Character.isDigit(D.charAt(1))) {
                    ...
                    int mm   = (tmm >= 1)? tmm : (useCtxStart||useDayStart)? 0 : (useCtxEnd||useDayEnd)? 59 : dt.getMinute(); // day-start change minute 1==>0 [2.6.7-B21b]
                    ...
                    switch (lastCh) {
                        ...
                        case 'm': { // months (ambiguous with 'minutes')
                            ...
                            int hh   = thh; // guaranteed
                            int mm   = (tmm >= 1)? tmm : (useCtxStart||useDayStart)? 0 : (useCtxEnd||useDayEnd)? 59 : now.getMinute(); // day-start change minute 1==>0 [2.6.7-B21b]
    ...
    **/
    ...
    /**
    *** Parse ISO_8601 Duration values and return represented duration in hours. <br>
    ...
    **/
    ...
    /**
    *** Parse ISO_8601 Duration values and return represented duration in seconds. 
    **/
    public static long ParseDurationSeconds(String durStr)
    {
        double hours = DateTime.ParseDurationHours(durStr);
        if (hours < 0.0) {
            return -1L;
        } else {
            return (long)(hours * 3600.0);
        }
    }
    ...
    **/
    ...
    /**
    *** Parse offset into seconds divided by second-units
    **/
    public static double parseOffset(String v, double secDiv, double dft)
    {
        ...
        /* parse/extract value */
        double ofs = StringTools.parseDouble(fn.getValueString(), Double.NaN);
        if (Double.isNaN(ofs)) {
            // -- unable to parse
            return dft;
        }

        /* convert parsed value to seconds */
        String t = fn.getTrailingString().toLowerCase();
        if (StringTools.isBlank(t)) {
            // -- assume already in preferred units
            return (ofs);
        } else
        if (t.equals("y") || t.equals("year") || t.equals("years")) {
            // -- years (approximate)
            return (ofs * (double)SECONDS_PER_YEAR) / secDiv;
        } else
        if (t.equals("w") || t.equals("week") || t.equals("weeks")) {
            // -- weeks
            return (ofs * (double)SECONDS_PER_WEEK) / secDiv;
        } else
        if (t.equals("d") || t.equals("day") || t.equals("days")) {
            // -- days
            return (ofs * (double)SECONDS_PER_DAY) / secDiv;
        } else
        if (t.equals("h") || t.equals("hour") || t.equals("hours")) {
            // -- hours
            return (ofs * (double)SECONDS_PER_HOUR) / secDiv;
        } else
        if (t.equals("m") || t.equals("min") || t.equals("minute") || t.equals("minutes")) {
            // -- minutes
            return (ofs * (double)SECONDS_PER_MINUTE) / secDiv;
        } else
        if (t.equals("s") || t.equals("sec") || t.equals("second") || t.equals("seconds")) {
            // -- seconds
            return (ofs) / secDiv;
        } else
        if (t.equals("ms") || t.equals("millisecond") || t.equals("milliseconds")) {
            // -- milliseconds
            return (ofs / 1000.0) / secDiv;
        } else {
            // -- assume already in preferred units
            return (ofs);
        }
    ...
    **/
    public static long parseOffsetSeconds(String v, long dft)
    {
        return (long)DateTime.parseOffset(v, 1.0, (double)dft);
    }

    /**
    *** Parse offset into hours
    **/
    public static double parseOffsetHours(String v, double dft)
    {
        return DateTime.parseOffset(v, (double)SECONDS_PER_HOUR, dft);
    }
    ...
    **/
    ...
    /**
    *** Constructor.  [2.6.7-B13h]
    *** @param tz       The TimeZone
    *** @param timeSec  The Epoch time in seconds
    **/
    public DateTime(TimeZone tz, long timeSec)
    {
        this(timeSec);
        this.setTimeZone(tz);
    }
    ...
    *** Returns true if the this DateTime is <b>after</b> the specified DateTime instance
    *** @param dt the other DateTime instance
    *** @param inclusive  True to include date/time, false to exclude
    *** @return True if this DateTime is after the specified DateTime instance.
    **/
    ...
    /**
    *** Returns true if the this DateTime is <b>after</b> the specified timestamp
    *** @param tsec The timestamp in seconds
    *** @param inclusive  True to include timestamp, false to exclude
    *** @return True if this DateTime is after the specified timestamp
    **/
    public boolean isAfter(long tsec, boolean inclusive)
    {
        if (inclusive) {
            return (this.getTimeSec() >= tsec);
        } else {
            return (this.getTimeSec() >  tsec);
        }
    }
    ...
    **/
    ...
    /**
    *** Returns true if the this DateTime is <b>before</b> the specified timestamp
    *** @param tsec The timestamp in seconds
    *** @param inclusive  True to include timestamp, false to exclude
    *** @return True if this DateTime is <b>before</b> the specified timestamp
    **/
    public boolean isBefore(long tsec, boolean inclusive)
    {
        if (inclusive) {
            return (this.getTimeSec() <= tsec);
        } else {
    ...
    **/
    ...
    {
        return this.toString(null, tmz);
    }

    /**
    *** Returns a String representation of this DateTime instance
    **/
    public String toString(String dtFmt) 
    {
        return this.toString(dtFmt, null);
    }

    /**
    *** Returns a String representation of this DateTime instance
    **/
    public String toString(String dtFmt, TimeZone tmz) 
    {
        // -- default format
        if (StringTools.isBlank(dtFmt)) {
            if (DateTime.simpleFormatter == null) {
                // -- eg. "Sun Mar 26 12:38:12 PST 2006" (not parsable by "parseArgumentDate")
                // -- eg. "yyyy/MM/dd,HH:mm:ss,zzz"
                DateTime.simpleFormatter = new SimpleDateFormat(TOSTRING_DATE_FORMAT, Locale.US);
            }
            synchronized (DateTime.simpleFormatter) {
                DateTime.simpleFormatter.setTimeZone(this._timeZone(tmz));
                return DateTime.simpleFormatter.format(this.getDate());
            }
        }
        // -- custom format
        return this.format(dtFmt, tmz);
    }
    ...
    **/
    ...
    //{
    //    java.util.Date dt = new java.util.Date(timeSec * 1000L);
    //    return dt.toString();
    //}
    ...
    **/
    public static void main(String argv[])
    {
        ...
        long deltaSec  = timeSec - nowSec;
        String deltaS  = ((deltaSec > 0)?"+":"") + String.valueOf(deltaSec);
        Print.sysPrintln("Sec from Now : "+deltaS);
