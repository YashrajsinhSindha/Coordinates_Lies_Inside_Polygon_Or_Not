...
//     -Added "isPrintableASCII(String s ...)"
//     -Added isBoolean_TRUE(..), isBoolean_FALSE(..)
//     -Updated "join" methods to convert a Class to a String via "className(...)"
//  2020/02/19  GTS Development Team
//     -Added exponent parsing support to "FilterNumber" class [2.6.7-B34d]
// ----------------------------------------------------------------------------
...
public class StringTools
{
  ...
  //private static final String DefaultCharEncoding     = CharEncoding_8859_1;
    ...
    **/
    ...
    {
        return StringTools.isPrintableASCII(b,0,-1,true/*inclSpace*/);
    }
    ...
    **/
    ...
    /**
    *** Returns true if the specified byte contains only printable ASCII characters
    *** @param b          The byte tested for printable ASCII
    *** @param inclSpace  True to include space characters (' ', '\t', '\n', '\r'), false to omit.
    *** @return True if the specified byte contains only printable ASCII characters
    **/
    public static boolean isPrintableASCII(byte b, boolean inclSpace)
    {
    ...
    /**
    *** Returns true if the specified String contains only printable ASCII characters
    *** @param s          The String tested for printable ASCII
    *** @param inclSpace  True to include space characters (' ', '\t', '\n', '\r'), false to omit.
    *** @return True if the specified String contains only printable ASCII characters
    **/
    public static boolean isPrintableASCII(String s, boolean inclSpace)
    {
        byte b[] = (s != null)? s.getBytes() : null;
        return StringTools.isPrintableASCII(b,inclSpace);
    }
    ...
    **/
    ...
    {
        byte b[] = (s != null)? s.getBytes() : null;
        return StringTools.isPrintableASCII(b);
    }
    ...
    *** Trims the leading/trailing blanks from the specified char array argument.<br>
    *** Similar to the String 'trim()' method with the addition that if the argument
    *** is null, a non-null empty char array will be returned.
    *** @param str  The char array to trim, or null to return an empty char array
    *** @return The trimmed String
    **/
    public static char[] trim(char cha[])
    {
        ...
        int s = 0; 
        for (;s < cha.length; s++) {
            if (!Character.isWhitespace(cha[s])) {
                break;
            }
        }
        // -- 's' points to first non-space char, or length if no first non-space chars
        if (s >= cha.length) {
            // -- 'cha' contains all space
            return new char[0];
        }

        /* find last non-space */
        int e = cha.length - 1;
        for (;e > s; e--) { // (e > s) sufficient because cha[s] is already confirmed non-space
            if (!Character.isWhitespace(cha[e])) {
                break;
            }
        }
        // -- 'e' points to last non-space char
        if (e < s) {
            // -- will not occur (cha[s] confirmed non-space), but test anyway
            return new char[0];
        }

        /* trim [ie. substring(s,e+1)] */
        int len = (e - s) + 1;
        if (len == cha.length) {
            // -- array is already trimmed
            return cha;
        } else {
            // -- remove leading/trailing spaces
            char nch[] = new char[len];
            System.arraycopy(cha,s,nch,0,len);
            return nch;
        }
    ...
    **/
    ...
    /**
    *** Returns true if the specified argument is null, or contains all whitespace characters.
    *** @param s  The String to test for blank.
    *** @return True if the specified argument is blank, or contains all whitespace characters.
    **/
    ...
    /**
    *** Returns true if the specified argument is null, or all whitespace characters.
    ...
    **/
    ...
    /**
    *** Returns true if the specified argument is null, or the "toString" value contains all whitespace characters.
    ...
    *** @return True if the specified argument is blank, or contains all whitespace characters.
    **/
    public static boolean isBlank(char cha[])
    {
        if (cha == null) {
            return true;
        } else {
    ...
    **/
    ...
    {
        return ListTools.equals(c1, c2);
    }

    /** (may not be fully tested)
    *** Returns true if the 2 char-arrays are equals (or are both null), false otherwise
    *** @return True if the specifed char-arrays are equals, or are both null.
    **/
    public static boolean equalsIgnoreCase(char c1[], char c2[])
    {
        if ((c1 == null) && (c2 == null)) {
            return true; // equals (both are null)
        } else
        if ((c1 == null) || (c2 == null)) {
            return false; // !equals (one of these is non-null)
        } else
        if (c1.length != c2.length) {
            ...
            int len = c1.length;
            for (int i = 0; i < len; i++) {
                if (Character.toUpperCase(c1[i]) != Character.toUpperCase(c2[i])) { 
                    return false; 
                }
            }
            return true;
    ...
    **/
    public static long parseLong(Object data, long dft)
    {
        ...
        if (data instanceof byte[]) {
            // -- could be ambiguous with "parseLong(Object data[], long dft)"
            byte b[] = (byte[])data;
            return StringTools.parseLong(b,0,b.length,true/*BE*/,false/*+/-*/,dft); // PayLoad.
        } else
    ...
    **/
    ...
    /**
    *** Parse the specified object into a byte value
    *** @param data  The object to parse
    *** @param dft   The default byte value if unable to parse the specified object
    *** @return The parsed byte value
    **/
    public static byte parseByte(Object data, byte dft)
    {
        if (data == null) {
            return dft;
        } else
        if (data instanceof java.util.concurrent.atomic.AtomicInteger) {
            return (byte)((java.util.concurrent.atomic.AtomicInteger)data).get();
        } else
        if (data instanceof java.util.concurrent.atomic.AtomicLong) {
            return (byte)((java.util.concurrent.atomic.AtomicLong)data).get();
        } else
        if (data instanceof Number) {
            return ((Number)data).byteValue();
        } else
        if (data instanceof Boolean) {
            return ((Boolean)data).booleanValue()? (byte)1 : (byte)0;
        } else
        if (data instanceof AccumulatorBoolean) {
            return ((AccumulatorBoolean)data).get()? (byte)1 : (byte)0;
        } else {
            return StringTools.parseByte(data.toString(), dft);
        }
    }

    /**
    *** Parse the specified String into a byte value
    *** @param data  The String to parse
    *** @param dft   The default byte value if unable to parse the specified object
    *** @return The parsed byte value
    **/
    public static byte parseByte(String data, byte dft)
    {
        return StringTools.parseByte(new FilterNumber(data, Byte.class), dft);
    }

    /**
    *** Parse the specified FilterNumber into a byte value
    *** @param num  The FilterNumber to parse
    *** @param dft  The default byte value if unable to parse the specified object
    *** @return The parsed byte value
    **/
    public static byte parseByte(FilterNumber num, byte dft)
    {
        if ((num != null) && num.supportsType(Byte.class)) {
            if (num.isHex()) {
                return (byte)StringTools.parseHexLong(num.getValueString(), dft);
            } else {
                try {
                    return Byte.parseByte(num.getValueString());
                } catch (NumberFormatException nfe) {
                    // Since 'FilterNumber' makes sure that only digits are parsed,
                    // this likely means that the specified digit string is too large
                    // for this required data type.  Our last ditch effort is to
    ...
    /**
    *** Return true if the specified String contains a valid byte value
    *** @param data   The String to test
    *** @param strict True to test for a strict byte value (ie. does not contain
    ***               any other superfluous trailing characters), false to allow for 
    ***               other non-critical trailing characters.
    *** @return True if the specified String contains a valid byte value
    **/
    public static boolean isByte(String data, boolean strict)
    {
        if (StringTools.isBlank(data)) {
            return false;
        } else {
            FilterNumber fn = new FilterNumber(data, Byte.class);
            return fn.isValid(strict);
        }
    }
    
    /**
    *** Return true if the specified Object contains a valid byte value
    *** @param data   The Object to test
    *** @param strict True to test for a strict byte value (ie. does not contain
    ***               any other superfluous trailing characters), false to allow for 
    ***               other non-critical trailing characters.
    *** @return True if the specified String contains a valid byte value
    **/
    public static boolean isByte(Object data, boolean strict)
    {
        if (data == null) {
            return false;
        } else
        ...
        if (data instanceof AccumulatorBoolean) {
            ...
            return StringTools.isByte(data.toString(), strict);
        }
    }
    ...
    **/
    public static BigInteger parseBigInteger(Object data, BigInteger dft)
    {
        ...
        if (data instanceof byte[]) {
            // -- may be ambiguous with "parseBigInteger(Object data[],...)" (if it is implemented)
    ...
    **/
    public static class FilterNumber
    {
        ...
        public FilterNumber(String val, Class<?> type) {
            ...
            boolean foundDigit = (p > ps); // eg. false for ".000"
            ...
            if ((type != null) && (
                ...
                // -- Long/Integer/Short/Byte/BigInteger, do not check for decimal or exponent
                num = v.substring(s, p);
            } else {
                ...
                if (v.charAt(p) == '.') {
                    ...
                    p++; // skip '.'
                    for (ps = p; (p < vlen) && Character.isDigit(v.charAt(p));) { p++; } // consume decimal
                    if (p > ps) { foundDigit = true; }
                }
                // -- Double/Float exponent?
                if ((p < vlen) && (Character.toUpperCase(v.charAt(p)) == 'E')) {
                    this.hasExponent = true;
                    p++; // skip 'E'
                    if ((p < vlen) && ((v.charAt(p) == '-') || (v.charAt(p) == '+'))) { p++; } // skip +/-
                    for (;(p < vlen) && Character.isDigit(v.charAt(p));) { p++; } // consume exponent
                }
                // -- extract double/float value (eq. "45.60E+2")
                num = v.substring(s, p);
            }
            ...
            if (foundDigit) {
                ...
                this.endPos     = p;
                /*
                Print.logInfo("IsHex    : " + this.isHex);
                Print.logInfo("InputStr : " + this.inpStr);
                Print.logInfo("Type     : " + StringTools.className(this.type));
                Print.logInfo("NumStr   : " + this.numStr);
                Print.logInfo("StartPos : " + this.startPos);
                Print.logInfo("EndPos   : " + this.endPos);
                */
            }
        ...
        */
            ...
            if ((this.numStr != null) && (this.type != null)) {
                try {
                    ...
                    if (BigDecimal.class.equals(this.type)) {
                        ...
                        String typeS = StringTools.className(this.type);
                        Print.logError("Unkrecognized Number type: " + typeS);
                        return dft;
                    }
                } catch (NumberFormatException nfe) {
                    // -- should not occur
                    String typeS = StringTools.className(this.type);
                    Print.logException("Number conversion error [" + typeS + "]: " + this.numStr, nfe);
                    return dft;
                }
            }
            return dft;
        ...
        public String getTrailingString() {
            String s = this.getInputString();
            int e = this.getEnd();
            if (e < s.length()) {
                return s.substring(e).trim();
            } else {
                return "";
            }
        }
    ...
    **/
    public static boolean parseBoolean(String data, boolean dft)
    {
        if (data != null) {
            ...
            if (dft) {
                ...
                return StringTools.isBoolean_FALSE(v,false)? false : true/*dft*/;
            } else {
                // -- if default is 'false', only test for 'true'
                return StringTools.isBoolean_TRUE(v,false)? true : false/*dft*/;
            }
        }
        return dft;
    }
    ...
    /**
    *** Returns true if the specified String contains a valid boolean "true" value
    *** @param data  The String to test
    *** @param strict True to test for a strict boolean value (ie. does not contain
    ***               any other superfluous trailing characters), false to allow for 
    ***               other non-critical trailing characters.
    **/
    public static boolean isBoolean_TRUE(String data, boolean strict)
    {
        if (data != null) {
            String v = data.toLowerCase();
    ...
    /**
    *** Returns true if the specified String contains a valid boolean "false" value
    *** @param data  The String to test
    *** @param strict True to test for a strict boolean value (ie. does not contain
    ***               any other superfluous trailing characters), false to allow for 
    ***               other non-critical trailing characters.
    **/
    public static boolean isBoolean_FALSE(String data, boolean strict)
    {
        if (data != null) {
            String v = data.toLowerCase();
    ...
    **/
    ...
    {
        if (StringTools.isBoolean_TRUE(data,strict)) {
            return true;
        } else
        if (StringTools.isBoolean_FALSE(data,strict)) {
            return true;
        } else {
            return false;
        }
    ...
    **/
    ...
    /** (may not be fully tested)
    *** Returns true if the specified target char-array starts with the pattern String
    *** @param t The target char-array
    *** @param p The pattern String
    *** @return True if the target String starts with the pattern String, false otherwise.
    **/
    public static boolean startsWith(char tc[], String p)
    {
        ...
        if (tc.length < p.length()) {
            return false; // target is smaller than pattern
        }

        /* case-sensitive compare */
        char pc[] = p.toCharArray(); // assuming (pc.length == p.length())
    ...
    *** @return True if the target String starts with a pattern String, false otherwise.
    **/
    public static boolean startsWith(char tc[], String ap[])
    {

        /* quick checks */
        if ((tc == null) || (ap == null) || (ap.length <= 0)) {
            return false;
        }

        /* test all pattern Strings */
        for (String p : ap) {
            if (StringTools.startsWith(tc,p)) {
                return true;
            }
        }
        return false;
    ...
    *** @return True if the target String starts with the pattern String, false otherwise.
    **/
    public static boolean startsWithIgnoreCase(char tc[], String p)
    {
        ...
        char pc[] = p.toCharArray(); // assuming (pc.length == p.length())
        for (int i = 0; i < pc.length; i++) {
            char tci = Character.toLowerCase(tc[i]);
            char pci = Character.toLowerCase(pc[i]);
            if (tci != pci) {
                return false;
            }
        }
    ...
    *** @return True if the target String starts with a pattern String, false otherwise.
    **/
    public static boolean startsWithIgnoreCase(char tc[], String ap[])
    {
        ...
        /* test all pattern Strings */
        for (String p : ap) {
            if (StringTools.startsWithIgnoreCase(tc,p)) {
                return true;
            }
        }
        return false;
    ...
    **/
    ...
    /** (may not be fully tested)
    *** Returns true if the specified target char-array ends with the pattern String
    *** @param t The target char-array
    *** @param p The pattern String
    *** @return True if the target char-array ends with the pattern String, false otherwise.
    **/
    public static boolean endsWith(char tc[], String p)
    {
        ...
        /* case-sensitive compare */
        char pc[] = p.toCharArray(); // assuming (pc.length == p.length())
        int tofs = tc.length - pc.length; // will be >=0
        for (int i = 0; i < pc.length; i++) {
            char tci = tc[i + tofs];
            char pci = pc[i];
            if (tci != pci) {
    ...
    *** @return True if the target char-array ends with the pattern String, false otherwise.
    **/
    public static boolean endsWithIgnoreCase(char tc[], String p)
    {
        ...
        int tofs = tc.length - pc.length; // will be >=0
        for (int i = 0; i < pc.length; i++) {
            char tci = Character.toLowerCase(tc[i + tofs]);
            char pci = Character.toLowerCase(pc[i]);
            if (tci != pci) {
                return false;
            }
    ...
    /**
    *** Returns true if the specified target char-array ends with one of the pattern Strings in 
    *** the specified array, without regard to case.
    *** @param t  The target char-array
    *** @param ap An array of pattern Strings
    *** @return True if the test String ends with any pattern String, false otherwise.
    **/
    public static boolean endsWithIgnoreCase(char tc[], String ap[])
    {
        ...
        /* test all pattern Strings */
        for (String p : ap) {
            if (StringTools.endsWithIgnoreCase(tc,p)) {
                return true;
            }
        }
        return false;
    ...
    /**
    *** Original copied from String.java and copied here to add a method that
    *** should have been in the String.java class in the first place.
    *** @param   source       the characters being searched.
    *** @param   sourceOffset offset of the source string.
    *** @param   sourceCount  count of the source string.
    *** @param   target       the characters being searched for (pattern)
    ...
    ***/
        ...
        int fromIndex) 
    {
        ...
        if ((sourceCount < 0) || (sourceCount > source.length)) {
            sourceCount = source.length; // reset source size
        }
        if ((targetCount < 0) || (targetCount > target.length)) {
            targetCount = target.length; // reset pattern size
        }
        if (fromIndex >= sourceCount) { // *original
            return (targetCount == 0)? sourceCount : -1;
        }
        if (fromIndex < 0) { // *original
            fromIndex = 0;
        }
        if (targetCount == 0) { // *original
            return fromIndex;
        }
        if ((sourceCount - fromIndex) < targetCount) {
            return -1; // source is smaller than pattern
        }
    ...
    **/
    ...
    {
        // -- fixed to return minimum index of all patterns [2.6.7-B08]
    ...
    **/
    ...
    {
        return StringTools.indexOf(t, 0, pp);
    }
    ...
    ***         String does not exist within the target String.
    **/
    public static int indexOf(String t, int n, char... pp)
    {
        // -- fixed to return minimum index of all patterns [2.6.7-B08]
        ...
        /* single pass through target String characters */
        char tt[] = t.toCharArray();
        for (int ti = n; ti < tt.length; ti++) {
            for (char p : pp) {
                if (tt[ti] == p) {
                    return ti;
                }
    ...
    ***         character does not exist within the target String.
    **/
    public static int indexOf(String t, char... pp)
    {
        return StringTools.indexOf(t, 0, pp);
    }
    ...
    **/
    public static String[] parseArray(String s, char arrayDelim)
    {
        ...
        String a[] = ListTools.toArray(v, String.class);
        return a;
    ...
    **/
    public static String encodeArray(Object list[], int ofs, int max, char delim, boolean alwaysQuote)
    {
        ...
        if (list != null) {
            ...
            int _max = ((max >= 0) && (max <= list.length))? max : list.length;
            for (int i = _ofs; i < _max; i++) {
                //if (sb.length() > 0) { sb.append(delim); } <== [2.6.7-B45u] removed, skips leading delimiters
                if (i > _ofs) { sb.append(delim); } // [2.6.7-B45u]
    ...
    **/
    public static String encodeArray(Object list, int ofs, int max, char delim, boolean alwaysQuote)
    {
        ...
        if (list instanceof java.util.List) {
            ...
            Object A[] = L.toArray(new Object[L.size()]);
            return StringTools.encodeArray(A, ofs, max, delim, alwaysQuote);
        } else 
    ...
    *** character as the delimiter.  Null elements in the input String array are skipped/ignored.
    *** Leading blank elements are skipped/ignored.
    ...
    public static String join(String val[], int ofs, int len, char delim)
    {
        ...
        if (val != null) {
            ...
            for (int i = _ofs; i < _end; i++) {
                if (val[i] != null) {
                    // -- Note: this currently has the effect of skipping all leading separators
                    // -- until at least the first non-blank entry is copied into the StringBuffer
                    if (sb.length() > 0) { sb.append(delim); }
                    sb.append(val[i]);
                }
            }
    ...
    **/
    public static String join(Object val[], int ofs, int len, char delim)
    {
        ...
        if (val != null) {
            ...
            for (int i = _ofs; i < _end; i++) {
                if (sb.length() > 0) { sb.append(delim); }
                if (val[i] instanceof Class) {
                    sb.append(StringTools.className((Class<?>)val[i])); // [2.6.6-B70]
                } else
                if (val[i] != null) {
            ...
            for (Iterator<?> i = list.iterator(); i.hasNext();) {
                ...
                if (ofs > 0) {
                    ...
                    if (val instanceof Class) {
                        sb.append(StringTools.className((Class<?>)val)); // [2.6.6-B70]
                    } else
                    if (val != null) {
    ...
    *** If the length of the String is equal-to, or greater-than, the specified length,  
    ...
    **/
    ...
    /**
    *** Pads the input String in the center with specified number of pad characters.  
    *** If the length of the String is equal-to, or greater-than, the specified length,  
    *** then the input String is returned as-is.
    *** @param s  The input String
    *** @param padChar  The pad character
    ...
    *** @return The padded String
    **/
    public static String padCenter(String s, char padChar, int len)
    {
        String padChS = String.valueOf(padChar);
        int    absLen = Math.abs(len);
        if (s == null) {
            return StringTools.replicateString(padChS, absLen);
        } else
        if (s.length() >= absLen) {
            return s;
        } else {
            int    padLen   = absLen - s.length();
            int    prePadL  = (len > 0)? (padLen / 2) : (padLen - (padLen / 2));
            int    postPadL = padLen - prePadL;
            String prePad   = StringTools.replicateString(padChS, prePadL);
            String postPad  = StringTools.replicateString(padChS, postPadL);
            return prePad + s + postPad;
        }
    }

    /**
    *** Pads the input String in the center with specified number of ' ' characters.
    *** If the length of the String is equal-to, or greater-than, the specified length,  
    *** then the input String is returned as-is.
    *** @param s  The input String
    *** @param len The length up to which ' ' characters will be pre-pended
    *** @return The padded String
    **/
    public static String centerAlign(String s, int len)
    {
        return StringTools.padCenter(s, ' ', len);
    }

    /**
    *** See StringTools.centerAlign(String,int)
    **/
    ...
    /**
    *** Pads the input Long in the center with specified number of ' ' characters. 
    *** If the length of the String is equal-to, or greater-than, the specified length,  
    *** then the input String is returned as-is.
    *** @param s  The input Long value
    *** @param len The length up to which ' ' characters will be pre-pended
    *** @return The padded String
    **/
    public static String centerAlign(long s, int len)
    {
        return StringTools.padCenter(String.valueOf(s), ' ', len);
    }

    /**
    *** See StringTools.centerAlign(long,int)
    **/
    public static String centerJustify(long s, int len)
    {
        return StringTools.padCenter(String.valueOf(s), ' ', len);
    }
    ...
    **/
    ...
    /* pattern type prefixes (must remain lower-case) */
    public static final String REGEX_PREFIX     = "regex:";
    public static final String GLOB_PREFIX      = "glob:";

    /**
    *** Converts the "GLOB" pattern match specification to "REGEX".
    *** Accepts the following pattern values:<br>
    ***   - GLOB_PATTERN <br>
    ***   - glob:GLOB_PATTERN <br>
    ***   - regex:REGEX_PATTERN (returns REGEX_PATTERN) <br>
    **/
    public static String getRegexFromGlobPattern(String pattern)
    {
        ...
        if (pattern.startsWith(REGEX_PREFIX)) {
            ...
            return pattern.substring(REGEX_PREFIX.length());
        }

        // --------------------------------------
        // -- assume GLOB below
        String globPattern = pattern.startsWith(GLOB_PREFIX)?
            pattern.substring(GLOB_PREFIX.length()) : // remove "glob:" prefix
            pattern;

        /* convert "globPattern" to regex */
        StringBuffer sb = new StringBuffer();
        ...
        int inGroup = 0;
        int inClass = 0;
        int firstNdxInClass = -1;
        char[] chArr = globPattern.toCharArray();
        globPatternChars:
        for (int i = 0; i < chArr.length; i++) {
            ...
            switch (ch) {
                case '\\': // escape
                    ...
                    if (i >= chArr.length) {
                        sb.append(ch); // at end of String, save char and exit
                        break globPatternChars; // we are done
                    } else {
                        char nextCH = chArr[i]; // char following '\'
                        switch (nextCH) {
                            case ',':
                                // -- escape not needed
                                break;
                ...
                case '@':
                case '%':
                    if ((inClass == 0) || ((firstNdxInClass == i) && (ch == '^'))) {
                        sb.append("\\");
                    }
                    sb.append(ch);
                    break; // switch
                case '!':
                    sb.append((firstNdxInClass == i)? "^" : "!");
                    break; // switch
                case '{':
                    inGroup++;
                    sb.append("(");
    ...
    /**
    *** Compiles a regular expression, and returns a 'Pattern' instance.
    *** @return The 'Pattern' instance, or null if an error occurs
    **/
    public static Pattern regexCompile(String regex, int flags)
    {
        if (regex == null) {
            Print.logError("Invalid Regular Expression: " + regex); // null
        } else {
            try {
                return Pattern.compile(regex, flags);
            } catch (PatternSyntaxException pme) {
                Print.logError("Invalid Regular Expression: " + regex + " ["+pme+"]");
            } catch (IllegalArgumentException iae) {
                Print.logError("Invalid Flag values: 0x" + StringTools.toHexString(flags,32) + " ["+iae+"]");
            } catch (Throwable th) {
                Print.logError("Pattern compile error: " + regex + " ["+th+"]");
            }
        }
        // -- invalid regex/flags
        return null;
    ...
    **/
    public static String regexReplace(String target, String regex, String val)
    {
        ...
        } catch (Throwable th) { // PatternSyntaxException, IllegalArgumentException
    ...
    ***     False if the target or regular expression String is null, or if the 
    ***     target String does not match the regular expression.
    **/
    public static boolean regexMatches(String target, String regex)
    {
        ...
        try {
            ...
            Print.logException("Pattern match error", pme);
            return false;
        } catch (Throwable th) {
            Print.logException("Pattern match error", th);
            return false;
    ...
    ***     target String does not match the regular expression.
    **/
    public static boolean regexMatches(String target, Pattern regexPattern)
    {
        // -- invalid arguments
        if (target == null) {
            return false;
    ...
    **/
    ...
    /**
    *** Filter and return all Strings in the Collection that match the Glob pattern
    **/
    public static Collection<String> filterGlob(Collection<String> targetList, String glob)
    {
        String regex = StringTools.getRegexFromGlobPattern(glob);
        return StringTools.filterRegex(targetList, regex, null);
    }

    /**
    *** Filter and return all Strings in the Collection that match the Glob pattern
    **/
    public static Collection<String> filterGlob(Collection<String> targetList, String glob, Collection<String> _filterList)
    {
        String regex = StringTools.getRegexFromGlobPattern(glob);
        return StringTools.filterRegex(targetList, regex, _filterList);
    }

    /**
    *** Filter and return all Strings in the array that match the Glob pattern
    **/
    public static String[] filterGlob(String targArr[], String glob)
    {
        String regex = StringTools.getRegexFromGlobPattern(glob);
        return StringTools.filterRegex(targArr, regex);
    }

    // --------------------------------

    /**
    *** Filter and return all Strings in the Collection that match the Regex pattern
    **/
    public static Collection<String> filterRegex(Collection<String> targetList, String regex)
    {
        return StringTools.filterRegex(targetList, regex, null);
    }
    ...
    *** Filter and return all Strings in the Collection that match the Regex pattern
    **/
    public static Collection<String> filterRegex(Collection<String> targetList, String regex, Collection<String> _filterList)
    {
        // -- check parameters
        if (ListTools.isEmpty(targetList)) {
            return null;
        } else
        if (regex == null) {
            return null;
        ...
        // -- filter list
        Collection<String> filterList = (_filterList != null)? _filterList : new Vector<String>();
    ...
    /**
    *** Filter and return all Strings in the array that match the Regex pattern
    **/
    public static String[] filterRegex(String targArr[], String regex)
    {
        Collection<String> filt = StringTools.filterRegex(ListTools.toListWrapper(targArr), regex, null);
        return (filt != null)? ListTools.toArray(filt,String.class) : null;
    }
    ...
    ***  - {i[:f][c][=d]}  Where <b>i</b> refers to the numeric index of the item in the replacement array,
    ***                    <b>:f</b> refers to the field size in which the item will be placed 
    ***                    (<b>f</b> may be negative for right-aligned fields), 
    ***                    <b>c</b> may be a literal "c" to indicate that the field should be centered,
                    ...
                    if ((ndx >= 0) && (ndx < v.length)) {
                        ...
                        if (j > 0) {
                            if (r.length() > j) {
                                ...
                                return r.substring(0,j-1) + "*";
                            } else
                            if (StringTools.endsWithIgnoreCase(arg,"c")) {
                                // -- center in field (favor left of center?)
                                return StringTools.centerJustify(r,j);
                            } else {
                                // -- pad-right in field
                        ...
                        if (j < 0) {
                            if (r.length() > (-j)) {
                                // -- exceeds field length
                                return "*" + r.substring(r.length() - ((-j) - 1));
                            } else
                            if (StringTools.endsWithIgnoreCase(arg,"c")) {
                                // -- center in field (favor right of center?)
                                return StringTools.centerJustify(r,j);
                            } else {
                                // -- pad-left in field
    ...
    public static String format(long val, String fmt, int fieldSize)
    {
        ...
        if (fmt.startsWith("%")) {
            try {
                ...
                // -- IllegalFormatPrecisionException, IllegalFormatConversionException, UnknownFormatConversionException
                Print.logException("Format exception [" + fmt + "]", th);
                s = String.valueOf(val);
            }
        } else {
    ...
    **/
    ...
    public static final int ELAPSED_FORMAT_SS       = -1; // "SSSSS";       // "12345"
    public static final int ELAPSED_FORMAT_HHMMSS   =  0; // "HH:MM:SS";    // "01:23:45"
    public static final int ELAPSED_FORMAT_HHMM     =  1; // "HH:MM";       // "01:23"
    public static final int ELAPSED_FORMAT_HHHhh    =  2; // "HHH.hh";      // "1.34"
    public static final int ELAPSED_FORMAT_HHHh     =  3; // "HHH.h";       // "1.3"
    public static final int ELAPSED_FORMAT_MMMSS    =  4; // "MMM:SS";      // "83:45"
    public static final int ELAPSED_FORMAT_DDHHMMS  =  5; // "DD-HH:MM:SS"; // "1-01:23:45"
    ...
    ***     ELAPSED_FORMAT_DDHHMMS  : "DD-HH:MM:SS"<br>
    ...
    **/
    public static String formatElapsedSeconds(long elapsedSec, int fmt)
    {
        ...
        switch (fmt) {
            ...
            case ELAPSED_FORMAT_DDHHMMS: {
                // -- "12-17:34:26", "1-02:34:56", "07:23:47"
                int d = (int)(elapsedSec / (60L * 60L * 24L));  // Days
                int h = (int)((elapsedSec / (60L * 60L)) % 24); // Hours
                int m = (int)((elapsedSec / 60L) % 60);         // Minutes
                int s = (int)(elapsedSec % 60);                 // Seconds
                if (d > 0) {
                    sb.append(StringTools.format(d,"0"));
                    sb.append("-");
                }
    ...
    /**
    *** Returns the package name for the specified object.  Does not return null.
    *** @param c  The object for which the package name is returned.
    ***           If a String instance is specified, a warning will be displayed.
    *** @return The class name of the specified object.  If the specified object is a Class object, 
    ***         then 'getPackage().getName()' is used to return the package name directly.
    **/
    public static String packageName(Object c)
    {
        if (c == null) {
            return "null";
        } else
        if (c instanceof Class) {
            // -- find and return name of package
            Package p = ((Class)c).getPackage();
            if (p != null) { // may be null if unknown
                return p.getName();
            }
            // -- continue below
        } else {
            // -- warning for specifying a String
            if (c instanceof String) {
                // -- the caller probably did not intend to pass in a String
                // -  issue a warning and continue.
                Print.logWarn("String type specified, \"java.lang\" will be returned");
            }
            // -- find and return name of package
        ...
        // -- otherwise extract package name from classname of object
        String cn = StringTools.className(c);
        int cp = cn.lastIndexOf(".");
        return (cp >= 0)? cn.substring(0,cp) : "";
    }

    /**
    *** Returns the full-qualified class name for the specified object.  Does not return null.
    *** @param c  The object for which the class name is returned
    *** @return The class name of the specified object.  If the specified object is a Class object, 
    ***         then 'getName()' is used to return the class name directly.
    **/
    public static String className(Object c)
    {
        return StringTools.className(c,true); // FQCN (include package)
    }

    /**
    *** Returns the class name for the specified object.  Does not return null.
    *** @param c  The object for which the class name is returned
    *** @param inclPkg  True to include the package name (FQCN), False to omit the package name.
    *** @return The class name of the specified object.  If the specified object is a Class object, 
    ***         then 'getName()' is used to return the class name directly.
    **/
    public static String className(Object c, boolean inclPkg)
    {
        ...
        if (c.getClass().isArray()) {
            ...
            fqcn = c.getClass().getName();
        }

        /* include/remove package? */
        if (inclPkg) {
            // -- include package
            return fqcn;
        } else {
    ...
    **/
    ...
    /**
    *** Replace "mailto:" with html mail-to anchor
    *** @param text The text to encode "mailto:" requests
    *** @return     The encoded string.
    **/
    public static String htmlEncodeAnchorMailto(String text)
    {
        if (text == null) {
            return "";
        } else {
            ...
            while (true) {
                // -- look for "mailto:"
                int p = remain.indexOf("mailto:");
                if (p < 0) {
                    sb.append(remain);
                    break;
                }
                // -- extract "mailto:EMAIL"
                int s = StringTools.indexOf(remain, p, chTerm);
                if (s < p) { s = remain.length(); }
                String mailto = remain.substring(p,s);
                String email  = mailto.substring("mailto:".length());
                // -- convert and append to accumulator
                sb.append(remain.substring(0,p)); // everything before "mailto:"
                sb.append("<a href='" + mailto + "'>" + email + "</a>");
                remain = remain.substring(s); // everything following "mailto:EMAIL"
            }
            return sb.toString();
        }
    }
    ...
    **/
    ...
    /**
    *** Converts the specified String to the type based on the specified object class.
    *** @param sVal     The String value to convert to an Object of the specified type
    *** @param dataType The target Object class type
    *** @return An Object of specified type.  Returns null if unable to convert the String
    ***         to the specified data type.
    **/
    public static Object toObjectValue(String sVal, Class<?> dataType)
    {
        Object dft = null;
        String v = (sVal != null)? sVal : "";
        ...
        if (String[].class.equals(dataType)) {
            return StringTools.split(v,','); // assuming comma-separated
        } else
        if (char[].class.equals(dataType)) {
            return v.toCharArray();
        }

        /* Double */
        if (Double.class.equals(dataType) || Double.TYPE.equals(dataType)) {
            if (StringTools.isDouble(v,false)) {
                return new Double(StringTools.parseDouble(v,0.0));
            } else {
                //Print.logWarn("Not a Double: " + v);
                return dft;
            }
        } else
        if (AccumulatorDouble.class.equals(dataType)) {
            if (StringTools.isLong(v,false)) {
                return new AccumulatorDouble(StringTools.parseDouble(v,0.0));
            } else {
                //Print.logWarn("Not a Double: " + v);
                return dft;
            }
        ...
        /* Float */
        if (Float.class.equals(dataType) || Float.TYPE.equals(dataType)) {
            if (StringTools.isFloat(v,false)) {
                return new Float(StringTools.parseFloat(v,0.0F));
            } else {
                //Print.logWarn("Not a Float: " + v);
                return dft;
            }
        }
        ...
        if (Long.class.equals(dataType) || Long.TYPE.equals(dataType)) {
            if (StringTools.isLong(v,false)) {
                return new Long(StringTools.parseLong(v,0L));
            } else {
                //Print.logWarn("Not a Long: " + v);
                return dft;
            }
        } else
        if (java.util.concurrent.atomic.AtomicLong.class.equals(dataType)) {
            if (StringTools.isLong(v,false)) {
                return new java.util.concurrent.atomic.AtomicLong(StringTools.parseLong(v,0L));
            } else {
                //Print.logWarn("Not a Long: " + v);
                return dft;
            }
        } else
        if (AccumulatorLong.class.equals(dataType)) {
            if (StringTools.isLong(v,false)) {
                return new AccumulatorLong(StringTools.parseLong(v,0L));
            } else {
                //Print.logWarn("Not a Long: " + v);
                return dft;
            }
        ...
        /* Integer */
        if (Integer.class.equals(dataType) || Integer.TYPE.equals(dataType)) {
            if (StringTools.isInt(v,false)) {
                return new Integer(StringTools.parseInt(v,0));
            } else {
                //Print.logWarn("Not an Integer: " + v);
                return dft;
            }
        } else
        if (java.util.concurrent.atomic.AtomicInteger.class.equals(dataType)) {
            if (StringTools.isInt(v,false)) {
                return new java.util.concurrent.atomic.AtomicInteger(StringTools.parseInt(v,0));
            } else {
                //Print.logWarn("Not an Integer: " + v);
                return dft;
            }
        } else
        if (AccumulatorInteger.class.equals(dataType)) {
            if (StringTools.isInt(v,false)) {
                return new AccumulatorInteger(StringTools.parseInt(v,0));
            } else {
                //Print.logWarn("Not an Integer: " + v);
                return dft;
            }
        ...
        /* Short */
        if (Short.class.equals(dataType) || Short.TYPE.equals(dataType)) {
            if (StringTools.isShort(v,false)) {
                return new Short(StringTools.parseShort(v,(short)0));
            } else {
                //Print.logWarn("Not a Short: " + v);
                return dft;
            }
        }

        /* Byte/byte[] */
        if (Byte.class.equals(dataType) || Byte.TYPE.equals(dataType)) {
            if (StringTools.isByte(v,false)) {
                return new Byte(StringTools.parseByte(v,(byte)0));
            } else {
                //Print.logWarn("Not a Byte: " + v);
                return dft;
            }
        } else
        if (byte[].class.equals(dataType)) {
            byte b[] = StringTools.parseHex(v,null);
            if (b != null) {
                return b;
            } else {
                //Print.logWarn("Not a byte array: " + v);
                return dft;
            }
        }

        /* Boolean */
        if (Boolean.class.equals(dataType) || Boolean.TYPE.equals(dataType)) {
            if (StringTools.isBoolean(v,false)) {
                return new Boolean(StringTools.parseBoolean(v,false));
            } else {
                //Print.logWarn("Not a Boolean: " + v);
                return dft;
            }
        } else
        if (AccumulatorBoolean.class.equals(dataType)) {
            if (StringTools.isBoolean(v,false)) {
                return new AccumulatorBoolean(StringTools.parseBoolean(v,false));
            } else {
                //Print.logWarn("Not a Boolean: " + v);
                return dft;
            }
        ...
        /* Locale */
        if (Locale.class.equals(dataType)) {
            return I18N.getLocale(v);
        }

        /* DateTime/DayNumber/TimeZone */
        if (DateTime.class.equals(dataType)) {
            try {
                return DateTime.parseArgumentDate(v, null, false);
            } catch (DateTime.DateParseException dpe) {
                //Print.logError("Unable to parse DateTime: " + dpe);
                return dft;
            }
        } else
        if (DayNumber.class.equals(dataType)) {
            DayNumber dn = DayNumber.parseDayNumber(v);
            if (dn != null) {
                return dn;
            } else {
                //Print.logError("Unable to parse DayNumber: " + v);
                return dft;
            }
        } else
        if (TimeZone.class.equals(dataType)) {
            return DateTime.getTimeZone(v, DateTime.getGMTTimeZone());
        }

        /* GeoPoint */
        if (GeoPoint.class.equals(dataType)) {
            GeoPoint gp = new GeoPoint(v, GeoPoint.PointSeparatorChars); // '/' or ','
            if (gp.isValid()) {
                return gp;
            } else {
                //Print.logWarn("Not a valid GeoPoint: " + v);
                return dft;
            }
        }

        /* Enum */
        if (dataType.isEnum()) { // "Enum.class.isAssignableFrom(dataType)" will not work here
            Class<? extends Enum> enumType = (Class<? extends Enum>)dataType;
            Object e = EnumTools.getValueOf(enumType, v, null, null, false);
            if (e != null) {
                return e;
            } else {
                //Print.logWarn("Not an Enum: " + v);
                return dft;
            }
        }

        /* finally, try a String constructor */
        try {
            // -- try instantiating a String constructor
            MethodAction ma = new MethodAction(dataType,String.class);
            return ma.invoke(v);
        } catch (Throwable th) {
            Print.logError("Unable to convert to Object: " + th);
            return dft;
        }
    ...
    /**
    *** Erase String.
    *** This method will attempt to clear/zero the contents of the specified String.
    *** This should only be used as a security measure for erasing sensitive information 
    *** when a String is no longer useful.
    *** Note: This does violate the String immutability rule.
    *** @param s  The String to erase/clear
    *** @return True if successful, false otherwise
    **/
    public static boolean _eraseString(String s)
    {
        try {
            StringTools.__eraseString(s);
            return true;
    ...
    *** This method will attempt to clear/zero the contents of the specified String.
    *** This should only be used as a security measure for erasing sensitive information 
    *** from memory when a String is no longer useful.
    *** Warning: This works by attempting to clear the private variable "value" within the String object.
    ***     The side-effects of clearing a String private variable is not fully known.
    ***     Because of this, use of this method is not recomended.
    *** Notes: 
    ***     - This does violate the String immutability rule.
    ***     - In testing, this seems to also clear copies of the target string made with 
    ***       (new String(orig)), since they appear to share the same char[].
    ***     - Copies of the target String made with (new String(""+orig)) are not cleared.
    *** @param s  The String to erase/clear
    *** @throws SecurityException if unable to clear the String
    **/
    public static void __eraseString(String s)
    ...
    ***/
    public static void main(String argv[])
    {
        ...
        if (format != null) {
            ...
            if ((fmt[0].indexOf(".") >= 0) || (fmt[0].toUpperCase().indexOf("E") >= 0)) {
                // -- double
                double V = StringTools.parseDouble(fmt[0],0.0);
                if ((fmt.length > 1) && !StringTools.isBlank(fmt[1])) {
                    String F = fmt[1].replace('_',' ');
                    Print.sysPrintln("Formatted Value ["+fmt[0]+"]: \""+F+"\" ==>" + StringTools.format(V,F) + "<");
                } else {
                    Print.sysPrintln("Parsed Value ["+fmt[0]+"] ==> " + StringTools.parseDouble(V,0.0));
                }
            } else {
                // -- long
