...
import java.nio.file.attribute.FileTime; // Java 7+
import java.nio.file.attribute.BasicFileAttributes; // Java 7+
...
public class FileTools
{
    ...
    /* likely non-binary files */
    private static String NonBinaryFileExtensions[] = {
        "bat",
        "txt",
        "cpp",
        "cc",
        ...
        throws IOException
    {
        ...
        for (int R = 0; R < 2; R++) { // max 3 retry attempts (2 redirects)
            ...
            try {
                ...
                /* connect */
                urlConn.connect(); // possible NoRouteToHostException, IllegalArgumentException, etc.
            ...
            } catch (IOException ioe) { // eg: NoRouteToHostException, IllegalArgumentException
                ...
                throw new HTMLTools.HttpIOException(ioe, (HttpURLConnection)urlConn, null);
    ...
    public static void closeStream(InputStream in)
    {
        ...
        if (in == System.in) {
            ...
            try {
                ...
                //Print.logError("Unable to close stream: " + th);
            }
        }
    }
    ...
    *** @param dir          The parent directory
    *** @param acceptFilter The callback filter
    *** @return True if the specified file is a directory, false otherwise.
    **/
    public static boolean traverseAllFiles(File dir, FileFilter acceptFilter)
    {
        return FileTools.traverseAllFiles(dir, acceptFilter, null/*dirFilter*/);
    }
    ...
    *** @param dir          The parent directory
    *** @param acceptFilter The callback filter
    *** @param dirFilter    Filter used to decide if a given directory can be traversed
    *** @return True if the specified file is a directory, false otherwise.
    **/
    public static boolean traverseAllFiles(File dir, FileFilter acceptFilter, FileFilter dirFilter)
    {
        if ((dir == null) || !dir.isDirectory()) {
            // -- not a directory
            return false;
        } else
        if ((dirFilter != null) && !dirFilter.accept(dir)) {
            // -- specified directory is rejected by directory filter
            return false;
        } else {
            // -- "dir" is an accepted directory
            File fileDirs[] = dir.listFiles(); // all files/directories
            if (fileDirs != null) {
                // -- traverse all files/directories
                for (File fd : fileDirs) {
                    acceptFilter.accept(fd);  // callback
                    //if ((dirFilter != null) && !dirFilter.accept(fd)) {
                    //    // -- ignore this file/directory
                    //} else
                    if (fd.isDirectory()) {
                        // -- decend into directory
                        FileTools.traverseAllFiles(fd, acceptFilter, dirFilter);
                    }
                }
                return true;
            } else {
    ...
    **/
    public static Path/*Object*/ toPath(File file) // getFilePath
        throws UnsupportedOperationException, 
    ...
    /**
    *** Gets the millisecond value from the specified FileTime object.
    *** Will compile on Java-6, however requires Java-7 for valid results.
    *** Will return -1L, when running on Java-6
    **/
    private static long _getFileTimeMS(Object fileTime, long dft)
        throws UnsupportedOperationException
    {

        /* return null if no FileTime defined */
        if (fileTime == null) {
        ...
        /* get milliseconds */
        try {
            MethodAction filesTimeMA = new MethodAction(fileTime, "toMillis");
            Object obj = filesTimeMA.invoke();
            return StringTools.parseLong(obj,dft);
            ...
            throw new UnsupportedOperationException("Unknown error: " + th);
        }
    ...
    **/
               ...
               SecurityException
    {
        ...
        Object filePath = FileTools.toPath(file); // java.nio.file.Path
        if (filePath == null) {
    ...
    /**
    *** Gets the file creation time (in milliseconds)
    *** (required Java 7+ to compile/run)
    **/
    public static long getCreationTimeMS(File file)
    {
        if (file != null) {
            try {
                BasicFileAttributes attr = Files.readAttributes(file.toPath(), BasicFileAttributes.class);
                FileTime ft = (attr != null)? attr.creationTime() : null;
                return (ft != null)? ft.toMillis() : -1L;
            } catch (IOException ioe) {
                Print.logError("File: " + ioe);
                return -1L;
            }
        } else {
            return -1L;
    ...
    /**
    *** Gets the file creation time (in seconds)
    *** (required Java 7+ to compile/run)
    **/
    public static long getCreationTimeSec(File file)
    {
        long createTimeMS = FileTools.getCreationTimeMS(file);
        return (createTimeMS > 0L)? (createTimeMS / 1000L) : createTimeMS;
    }
    ...
    **/
        ...
        throws UnsupportedOperationException 
    {
        ...
        if (file != null) {
            try {
                Object symLink = FileTools.getAttribute(file, "basic:isSymbolicLink"); // Boolean
    ...
    **/
    ...
    /**
    *** Gets the CVS directory for the specified file
    **/
    public static File getCVSDirectory(File f)
    {

        /* ivalid file */
        if (f == null) {
            Print.logWarn("Null file/directory");
            return null;
        }
        ...
        if (f.isDirectory()) {
            ...
            Print.logWarn("Not a file/directory: " + f);
            return null;
        }

        /* canonical directory */
        File realDir = FileTools.getRealFile(dir);
        if (realDir == null) {
            Print.logWarn("Unable to determine canonical directory: " + dir);
        } else {
            dir = realDir;
        }
        ...
        if (dir.getName().equals("CVS")) {
            ...
            File d = new File(dir,"CVS");
            if (d.isDirectory()) {
                return d;
            } else {
                Print.logWarn("CVS directory not found: " + d);
                return null;
            }
        }
    ...
    /**
    *** Gets the CVS project home directory
    **/
    public static File getCVSProjectHome(File dir)
    {

        /* ivalid directory */
        File cvsDir = FileTools.getCVSDirectory(dir);
        if (cvsDir == null) {
            Print.logWarn("Invalid directory: " + dir);
            return null;
        }

        /* CVS/Respository file */
        File cvsRepoFile = new File(cvsDir, "Repository");
        if (!cvsRepoFile.isFile()) {
            Print.logWarn("CVS/Entries file not found: " + cvsRepoFile);
            return null;
        }
        String cvsRepo = StringTools.toStringValue(FileTools.readFile(cvsRepoFile)).trim();

        /* remove trailing repo directory */
        String prjDirS = cvsDir.getParentFile().toString();
        if (prjDirS.endsWith(cvsRepo)) {
            String s = prjDirS.substring(0,prjDirS.length() - cvsRepo.length());
            return new File(s);
        } else {
            Print.logWarn("Unable to determine project home directory: " + prjDirS + " [" + cvsRepo + "]");
            return null;
        }
    ...
    /**
    *** Read the current CVS/Entries
    **/
    public static Collection<File> getCVSEntries(File dir, boolean inclSubDir)
    {
        return FileTools.getCVSEntries(dir, inclSubDir, null);
    }

    /**
    *** Read the current CVS/Entries
    **/
    public static Collection<File> getCVSEntries(File dir, boolean inclSubDir, Collection<File> prjList)
    {
        ...
        /* CVS/Entries file */
        File cvsEntriesFile = new File(cvsDir, "Entries");
        if (!cvsEntriesFile.isFile()) {
            Print.logWarn("CVS/Entries file not found: " + cvsEntriesFile);
            return prjList;
        }
        String _cvsEntries = StringTools.toStringValue(FileTools.readFile(cvsEntriesFile));
        String cvsEntries[] = StringTools.split(_cvsEntries,'\n');

        /* make sure we have a project file list */
        if (prjList == null) {
            prjList = new Vector<File>();
        }

        /* iterate through lines */
        File prjDir = cvsDir.getParentFile();
        for (String cvsItem : cvsEntries) {
            if (StringTools.isBlank(cvsItem)) {
                continue;
            } 
            // -- 
            String c[] = StringTools.split(cvsItem,'/'); // embedded/escaped '/'?
            if ((c.length == 1) && c[0].equals("D")) {
                // -- quietly ignore (likely last entry in file)
                continue;
            } else
            if (c.length != 6) {
                Print.logWarn("Invalid Entry length["+c.length+"]: " + cvsItem + " ["+cvsEntriesFile+"]");
                continue;
            } 
                
            // --
            File f = new File(prjDir, c[1]);
            if (c[0].equals("D")) {
                // -- directory
                if (f.isDirectory()) {
                    prjList.add(f);
                    if (inclSubDir) {
                        FileTools.getCVSEntries(f, inclSubDir/*true*/, prjList); // -- recursive decent
                    }
                } else {
                    Print.logWarn("Expected a directory: " + f);
                }
            } else {
                // -- file
                if (f.isFile()) {
                    ...
                    Print.logWarn("Expected a file: " + f);
                }
            }
        }
    ...
    /**
    *** Returns a list of files in the first list that are not in the second list
    **/
    public static Collection<File> getFileList_NOT(Collection<File> A, Collection<File> B)
    {

        /* nothing in the seoncd list */
        if (ListTools.isEmpty(B)) {
            return A; // may also be null
        } else 
        if (ListTools.isEmpty(A)) {
            return A;
        }
        
        /* convert B to connonical list */
        Set<File> bSet = new HashSet<File>();
        for (File b : B) {
            File bc = FileTools.getRealFile(b);
            bSet.add((bc != null)? bc : b);
        }

        /* loop through files in A-list */
        Vector<File> notList = new Vector<File>();
        Aloop:
        for (File a : A) {
            File ac = FileTools.getRealFile(a);
            for (File bc : bSet) {
                if (ac.equals(bc)) {
                    continue Aloop;
                }
    ...
    private static final String ARG_SEARCH[]        = { "search"     , "grep"       };  // File,File,...
    private static final String ARG_MATCH_START[]   = { "matchStart" , "startMatch" };  // String\String\...
    private static final String ARG_MATCH_PRINT[]   = { "matchPrint" , "printMatch" };  // String\String\...
    private static final String ARG_MATCH_STOP[]    = { "matchStop"  , "stopMatch"  };  // String\String\...

    private static final String ARG_CVS[]           = { "cvsEntries" , "cvs"        };  // Directory
    ...
    **/
    public static void main(String argv[])
    {
        ...
        if (RTConfig.hasProperty(ARG_SEARCH)) {
            // -- get list of files
            Vector<File> grepFileList = new Vector<File>();
            for (String gfs : StringTools.parseArray(RTConfig.getString(ARG_SEARCH,""))) {
                if (!StringTools.isBlank(gfs)) {
                    File gf = new File(gfs);
                    if (gf.isFile()) {
                        grepFileList.add(gf);
                    } else
                    if (gf.isDirectory()) {
                        for (File gdf : FileTools.getFiles(gf)) {
                            grepFileList.add(gdf);
                        }
                    } else {
                        Print.sysPrintln("ERROR: invalid file specification: " + gf);
                        System.exit(99);
                    }
                }
            }
            File grepFile[] = grepFileList.toArray(new File[grepFileList.size()]);
            // -- parse match keys
            String matchStart[] = StringTools.split(RTConfig.getString(ARG_MATCH_START,null),'\\');
            String matchPrint[] = StringTools.split(RTConfig.getString(ARG_MATCH_PRINT,null),'\\');
            String matchStop[]  = StringTools.split(RTConfig.getString(ARG_MATCH_STOP ,null),'\\');
            if (ListTools.isEmpty(matchStart)) {
                Print.sysPrintln("ERROR: Start-Match not specified");
                System.exit(1);
            } else
            if (ListTools.isEmpty(matchPrint)) {
                Print.sysPrintln("ERROR: Print-Match not specified");
                System.exit(1);
            } else
            if (ListTools.isEmpty(matchStop)) {
                Print.sysPrintln("ERROR: Stop-Match not specified");
                System.exit(1);
            }
            for (int i = 0; i < matchStart.length; i++) {
                Print.sysPrintln("StartMatch " + i + ": " + matchStart[i]);
            }
            // -- match state
            int mstart = 0;
            // --
            for (File f : grepFile) {
                ...
                try {
                    ...
                    while (true) {
                        ...
                        line++;
                        // -- stop
                        if ((mstart > 0) && (matchStop != null)) {
                            for (String sm : matchStop) {
                                if (!StringTools.isBlank(sm) && StringTools.indexOfIgnoreCase(s,sm) >= 0) {
                                    if (sb != null) {
                                        Print.sysPrintln("Matched: " + sb);
                                    }
                                    mstart = 0;
                                    sb = null;
                                    continue;
                        ...
                        // -- start
                        if (mstart < matchStart.length) {
                            String sm = matchStart[mstart];
                            if (!StringTools.isBlank(sm) && StringTools.indexOfIgnoreCase(s,sm) >= 0) {
                                mstart++;
                                while ((mstart < matchStart.length) && StringTools.isBlank(matchStart[mstart])) { mstart++; }
                            }
                            if (mstart < matchStart.length) {
                                continue;
                            }
                        ...
                        // -- print
                        if (matchPrint != null) {
                            for (String pm : matchPrint) {
                                if (!StringTools.isBlank(pm) && StringTools.indexOfIgnoreCase(s,pm) >= 0) {
                                    Print.sysPrintln(f.getName() + ":" + line + "] " + s);
