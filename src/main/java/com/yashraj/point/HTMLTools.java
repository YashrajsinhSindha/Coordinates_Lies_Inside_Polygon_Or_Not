...
//     -Added "HTMLTools.HttpIOException" to allow obtaining the HTTP response code in
...
//     -Added "getErrorData()" to "HTMLTools.HttpIOException" to provide the data from 
//      "getErrorStream()" when HttpURLConnection returns an error response code.
// ----------------------------------------------------------------------------
...
public class HTMLTools
{
    ...
    public enum RequestMethod {
        ...
        RequestMethod(String n)     { nn = n; }
        public String  toString()   { return nn; }
        public boolean isGET()      { return this.equals(GET   ); }
        public boolean isPOST()     { return this.equals(POST  ); }
        public boolean isPUT()      { return this.equals(PUT   ); }
        public boolean isDELETE()   { return this.equals(DELETE); }
    };
  ...
  //public  static final byte   MAGIC_MOV[]                 = new byte[]{(byte)0x00,(byte)0x00,(byte)0x00,(byte)0x14,(byte)0x66,(byte)0x74,(byte)0x79,(byte)0x70,(byte)0x71,(byte)0x74,(byte)0x20,(byte)0x20}; // unverified
    ...
    public  static final String HEADER_ACCESS_CONTROL_ALLOW_ORIGIN  = "Access-Control-Allow-Origin"; // CORS policy
    ...
    **/
    ...
    /**
    *** Returns true if the HttpURLConnection specifies an HTTP error code >= HTTP_BAD_REQUEST
    **/
    private static boolean IsHttpError(HttpURLConnection hc)
        throws IOException
    {
        if (hc != null) {
            int ec = hc.getResponseCode();
            return (ec >= HttpURLConnection.HTTP_BAD_REQUEST)? true : false;
        } else {
            return false;
        }
    }
    ...
    **/
        ...
        extends BufferedInputStream
    {
        ...
        public HttpBufferedInputStream(HttpURLConnection hc, boolean onError) throws IOException {
            //hc.getInputStream() <-- will throw exception on >= HTTP_BAD_REQUEST
            super((onError && HTMLTools.IsHttpError(hc))? hc.getErrorStream() : hc.getInputStream());
            this.httpConnect = hc;
        }
    ...
    **/
        ...
        extends IOException
    {
        ...
        public HttpIOException(IOException cause, int respCode, String respMsg, byte errorData[]) {
            super(cause);
        ...
        public HttpIOException(IOException cause, int respCode, String respMsg) {
            this(cause, respCode, respMsg, null);
        }
        public HttpIOException(IOException cause, int respCode) {
            this(cause, respCode, null);
        }
        public HttpIOException(IOException cause, HttpURLConnection hc, byte errorData[]) {
            super(cause);
            if (hc != null) {
                try { 
        ...
        public Throwable getCause() {
            return super.getCause(); // may be null
        }
        ...
        public String getMessage() {
            ...
            int    respCode = this.getResponseCode();     // "400"
            String  respMsg = StringTools.trim(this.getResponseMessage());  // "Bad Request"
            StringBuffer sb = new StringBuffer();
            if (respCode > 0) {
                sb.append(respCode);
                sb.append(" - ");
            }
            if (!StringTools.isBlank(respMsg)) {
                sb.append(respMsg);
                if (!StringTools.isBlank(causeMsg) && !respMsg.equals(causeMsg)) {
                    sb.append(" (").append(causeMsg).append(")");
                }
            } else
        ...
        public byte[] getErrorData() { // getResponseError
    ...
    **/
    ...
    public  static final String CONTENT_TYPE_JSON       = "application/json";        // [2.6.7-B14]
    public  static final String CONTENT_TYPE_JSON_REQ   = "application/jsonrequest"; // [2.6.7-B14]
    ...
    ***/
    public static byte[] readPage_POST(
        ...
        Properties headerProps, byte postData[], 
        int timeoutMS, int maxLen)
        throws IOException // NoRouteToHostException, HTMLTools.HttpIOException
    {
        return readPage_POST(
            pageURL, 
            headerProps, postData, 
    ...
    **/
        ...
        throws IOException // NoRouteToHostException, HTMLTools.HttpIOException
    {
        ...
        try {
            ...
            /* read data [readPage_POST] */
            //int contentLen = httpConnect.getContentLength();
            //if ((contentLen > 0) || (contentLen == -1)) {
            postInput = new HttpBufferedInputStream(httpConnect,true); // class HttpBufferedInputStream
            ...
            /* HTTP error code? */
            if (HTMLTools.IsHttpError(httpConnect)) {
                // -- will not get here if above "HttpBufferedInputStream" throws IOException
                throw new HTMLTools.HttpIOException(null, httpConnect, data); // class HTMLTools.HttpIOException
            }

        } catch (HTMLTools.HttpIOException hioe) {

            /* re-throw HTMLTools.HttpIOException */
            throw hioe;

        } catch (IOException ioe) {

            /* throw HTMLTools.HttpIOException if 'httpConnect' is available */
            if (HTMLTools.IsHttpError(httpConnect)) { // && (httpConnect != null)
                // -- read error bytes? [TODO:]
                byte errData[] = null;
                try {
                    postErrInp = httpConnect.getErrorStream();
                    ByteArrayOutputStream errOut = new ByteArrayOutputStream(); // no need to close
                    FileTools.copyStreams(postErrInp, errOut, maxLen);
                    errData = errOut.toByteArray();
                } catch (IOException err_ioe) {
                    // -- unable to read error stream
                }
                throw new HTMLTools.HttpIOException(ioe, httpConnect, errData); // class HTMLTools.HttpIOException
            }

            /* exception is not due to HTTP error code */
            throw ioe;

        } finally {

            /* close */
            if (postOutput  != null) { try{postOutput.close();      }catch(Throwable th){} }
            if (postInput   != null) { try{postInput.close();       }catch(Throwable th){} }
            if (postErrInp  != null) { try{postErrInp.close();      }catch(Throwable th){} }
            if (httpConnect != null) { try{httpConnect.disconnect();}catch(Throwable th){} }
    ...
    ***/
        ...
        throws IOException // HTMLTools.HttpIOException
    {
        ...
        try {            
            ...
            /* read data [readPage_GET] */
            //int contentLen = httpConnect.getContentLength();
            //if ((contentLen > 0) || (contentLen == -1)) {
            getInput = new HttpBufferedInputStream(httpConnect,true); // class HttpBufferedInputStream
            ByteArrayOutputStream output = new ByteArrayOutputStream(); // no need to close
            FileTools.copyStreams(getInput, output, maxLen);
            data = output.toByteArray(); // FileTools.readStream(getInput, maxLen);
            //} else {
            //    data = new byte[0];
            //}
            ...
            if (HTMLTools.IsHttpError(httpConnect)) { // && (httpConnect != null)
                ...
                byte errData[] = null;
                try {
                    getErrInp = httpConnect.getErrorStream();
                    ByteArrayOutputStream errOut = new ByteArrayOutputStream(); // no need to close
                    FileTools.copyStreams(getErrInp, errOut, maxLen);
                    errData = errOut.toByteArray();
                } catch (IOException err_ioe) {
                    // -- unable to read error stream
                }
            ...
            /* disconnect */
            if (getInput    != null) { try{getInput.close();        }catch(Throwable th){} }
            if (getErrInp   != null) { try{getErrInp.close();       }catch(Throwable th){} }
            if (httpConnect != null) { try{httpConnect.disconnect();}catch(Throwable th){} }
    ...
    **/
    public static HttpBufferedInputStream inputStream_GET(String pageURLStr, 
        Properties headerProps,
        int timeoutMS)
        throws IOException // HTMLTools.HttpIOException, MalformedURLException
    {
        URL pageURL = new URL(pageURLStr);
        ...
        throws IOException // HTMLTools.HttpIOException, MalformedURLException
    {
      ...
      //httpConnect.setRequestProperty("Accept-Language", "en-gb,en;q=0.5");
        ...
        try {
            ...
            /* read data [inputStream_GET] */
            int contentLen = httpConnect.getContentLength();
            if ((contentLen > 0) || (contentLen == -1)) {
                return new HttpBufferedInputStream(httpConnect,false); // class HttpBufferedInputStream
            } else {
            ...
            /* wrap in HTMLTools.HttpIOException */
            throw new HTMLTools.HttpIOException(spe, httpConnect, null);

        } catch (IOException ioe) {

            /* wrap in HTMLTools.HttpIOException */
            throw new HTMLTools.HttpIOException(ioe, httpConnect, null);
    ...
    **/
    public static void main(String argv[])
    {
        ...
        /* DEBUG/TEST: get image dimensions */
